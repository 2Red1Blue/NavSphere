...
界面决策与交互说明见 [Feed 阅读设计](docs/feed-design.md)。

- 🗞️ **紧凑阅读首页**：按日期组织单栏文章，中文无衬线标题控制在 20–22px，详情标题为 28–40px
- 🎛️ **阅读交互**：阅读／速览密度切换、就地展开导读、深浅色切换、300ms 搜索防抖与 `/` 搜索快捷键
- 🏛️ **专题空间**：原创透明展厅素材、六专题分组、空间／列表切换、专题搜索与独立阅读面板；移动端选中后定位到文章，素材失败仍可浏览
- 🌃 **赛博朋克风格**：深墨／霓虹青／电紫、扫描光带和交互边缘流光；支持关闭特效与系统减少动态效果，主题偏好与主站独立
- ✍️ **Writer Agent 成稿优先**：独立编辑稿位于阅读正文首位，AI 导读、推荐理由与评分移入编辑注侧栏
- 🔎 **可追溯阅读**：事实、分析、局限及证据编号继续使用固定结构渲染，并保留经验证的上游原文入口
- 📚 **内容导航**：最新动态、精选、日报、热点榜与专题共享 Feed 主题，不改变来源与发布权限
- ♿ **响应式与无障碍**：支持桌面和移动布局、键盘抽屉、Escape 关闭、阅读进度及 reduced-motion
...
短稿独立存入 `article_editorials`，阅读页显示“本站整理 / Writer Agent 成稿 · 独立新闻短稿”，与原文全文许可分开。标题、导语、事实、推断、局限和证据链接由固定结构渲染，不直接发布待审 Markdown。原始文章 ID、来源、评分及撤回状态不变。
