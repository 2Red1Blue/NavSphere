...
Physical size comes from D1's `meta.size_after`, not estimated body length. The script requires a successful result with zero written rows and `changed_db=false`. Missing/malformed metrics, timeouts and failed commands are errors, never a healthy zero-size database. A successful report includes a machine-consumable `capacityPolicy` snapshot: exact physical size, audit timestamp, all four fixed byte thresholds, `policyDecision`, `bytesToRelease` and `criticalAlert`. Exit 0 means the audit completed, **not** that compaction is enabled or safe; exit 2 means audit failure. The optional longer timeout is for management CLI startup/network latency and does not alter the site's production health gate.
...
The audit uses a conservative decimal 500,000,000-byte planning ceiling. These are fixed policy values, not Cloudflare limits:

| Physical D1 size | Decision | Target / alert |
| --- | --- | --- |
| Below 350,000,000 bytes (70%) | `hold` | No automated compact. |
| At or above 350,000,000 bytes | `compact` | Bounded candidate selection may release at most the bytes needed to reach 300,000,000 bytes (60%). |
| At or above 425,000,000 bytes (85%) | `compact` | `criticalAlert:true`; still stop at the same 300,000,000-byte target. |
| At or above 500,000,000 bytes | `compact` | `limit` status and critical alert; do not infer that body compaction will immediately shrink allocated pages. |

The `capacityPolicy` snapshot is accepted by the worker only when every field exactly matches this policy, `policyDecision` and release amount match the measured size, and `auditedAt` is no more than 24 hours old. A 30-day `--days` cutoff is only the minimum cooling period for a candidate; it never authorizes compacting by itself. There is no growth-rate forecast from one measurement. The report does not measure all account databases or account-wide daily quotas. A query's `rows_read` is the audit's own read cost; Wrangler's rolling-24-hour counters must not be labelled as current UTC-day remaining quota. DELETE and index updates consume write quota too.
...
- `compact`: separate server flag **and a fresh valid `capacityPolicy` with `policyDecision:'compact'`** are required. The article must also be past the configured minimum cooling period (30 days by default), have a pointer staged for at least24hours, pass a fresh verified KV read and byte-for-byte hot comparison, then a full snapshot CAS sets only `content=NULL`. No article/topic DELETE. The24h delay is conservative, not a guarantee of global KV availability; each clear still verifies the object. The API returns `CAPACITY_POLICY_REQUIRED`, `INVALID_CAPACITY_POLICY` or `CAPACITY_COMPACTION_NOT_REQUIRED` before any cold-body read or D1 clear when that capacity gate is not met.
...
# Keep this local report private to the operator workstation. It contains no bodies or credentials.
pnpm run cf:retention-audit > /secure/operator/d1-retention-audit.json
# Exact IDs are copied from the private operator inventory, never recomputed.
pnpm run cf:archive -- --mode compact --id <existing-article-id> --confirmed-recovery --confirmed-cold-reader --capacity-policy-file /secure/operator/d1-retention-audit.json
pnpm run cf:archive -- --mode rehydrate --id <existing-article-id>
```

The compact CLI refuses to start without the two recovery confirmations and `--capacity-policy-file`. It reads only the report's `capacityPolicy`, rejects expired, partial or internally inconsistent fields and a `hold` decision, then sends the exact snapshot in the authenticated request. For automatic batches it also stops candidate bytes at `bytesToRelease`, before the existing 20-item limit. `BATCH_COMPLETED` means only the selected bounded batch; `moreCandidates:true` means the inventory was truncated, candidates exceeded the capacity target, or more candidates exceeded the batch limit, not that every remaining item is ready to clear. Invalid dates are never eligible. Candidate queries use the shared strict ISO/calendar/timezone classifier, followed by TypeScript validation and a second per-item retention check.
