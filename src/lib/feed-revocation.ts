import { FEED_RETRY_AFTER_SECONDS, secureTokenEquals } from './feed-api'
import { ARCHIVE_COLUMNS, archiveSnapshotGuard, readArchiveBody } from './content-archive'
...
export async function restoreFeedFullText(
  ...
  try {
    const row = await db.prepare(
      `SELECT ${ARCHIVE_COLUMNS.join(', ')} FROM articles WHERE url_hash = ?`,
    ).bind(urlHash).first<ArchiveRow>()
    if (!row) return errorResponse('NOT_FOUND', 'Article not found', 404)
    if (row.approved_for_publication !== 1
    ...
    if (row.content === null) {
      if (row.content_archive_key === null) {
        return errorResponse('NO_VERIFIED_CONTENT', 'No verified full text is available', 409)
      }
      await readArchiveBody(kv, row)
    } else if (row.content.length === 0) {
      return errorResponse('NO_VERIFIED_CONTENT', 'No verified full text is available', 409)
    }

    // Storage reads cannot grant rights: only this D1 compare-and-swap can.
    // The revision trigger also detects a repeated same-second withdrawal.
    const guard = archiveSnapshotGuard(row)
    const result = await db.prepare(
      `UPDATE articles
       SET fulltext_publication_allowed = 1,
           fulltext_revoked_at = NULL
       WHERE ${guard.sql}`,
    ).bind(...guard.values).run()
    const changes = Number((result.meta as { changes?: unknown } | undefined)?.changes ?? 0)
    if (!result.success || !Number.isFinite(changes)) {
      return errorResponse('RESTORE_FAILED', 'Unable to restore full-text access', 503)
    }
    if (changes < 1) {
      return errorResponse('RESTORE_CONFLICT', 'Article changed; review current rights before retrying', 409)
    }
    return jsonResponse({ restored: true, url_hash: urlHash })
  } catch {
    console.error('NavSphere full-text restoration failed')
    return errorResponse('RESTORE_FAILED', 'Unable to restore full-text access', 503)
  }
}
